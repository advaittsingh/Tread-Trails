import type { AnalyticsDateRange } from "./date-range.js";
import { fillDailySeries } from "./date-range.js";
import type {
  AnalyticsReport,
  DailyConversion,
  RankedRow,
...
export async function buildAnalyticsReport(
  ...
  const [
    ...
    prisma.$queryRaw<
      { brand: string; units: number; revenue: number }[]
    >`
      SELECT
      ...
      LATERAL jsonb_array_elements(o.items::jsonb) AS line
      LEFT JOIN "Product" p
        ON p.slug = COALESCE(line->>'productSlug', line->>'slug')
      WHERE o."createdAt" >= ${start} AND o."createdAt" < ${endExclusive}
        AND o.status = 'paid'
      GROUP BY 1
      ORDER BY revenue DESC
    ...
    prisma.$queryRaw<{ service: string; count: number }[]>`
  ...
  const topBrands: RankedRow[] = topBrandRows.map((r) => ({
    key: r.brand,
    label: r.brand,
    count: num(r.units),
    revenue: num(r.revenue),
  ...
  return {
    ...
    topVehicles,
    bookings: {
      byStatus: bookingStatusRows.map((r) => ({
        key: r.status,
        label: r.status,
        count: num(r.count),
      })),
      byService: bookingServiceRows.map((r) => ({
        key: r.service,
        label: r.service,
        count: num(r.count),
      })),
...
export function analyticsReportToCsv(report: AnalyticsReport): string {
  ...
  push(["# Top brands (paid orders)"]);
  push(["brand", "units", "revenue"]);
  for (const b of report.topBrands) {
    push([b.label, b.count, b.revenue ?? 0]);
  }
  push([]);
  ...
  push(["# Bookings by status"]);
  push(["status", "count"]);
  for (const s of report.bookings.byStatus) {
    push([s.label, s.count]);
  }
  push([]);

  push(["# Bookings by service"]);
  push(["service", "count"]);
  for (const s of report.bookings.byService) {
    push([s.label, s.count]);
  }
  push([]);
