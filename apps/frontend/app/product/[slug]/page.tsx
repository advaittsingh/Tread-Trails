...
export default async function ProductDetailPage({ params }: Props) {
  ...
  const [related, bundleSuggestion] = await Promise.all([
    getRelatedProducts(product.slug),
    getBundleSuggestion(product.slug),
  ]);
  const bundlePack =
  ...
  const productSchema = seoBundle?.settings.productSchema;
  const productLd =
    productSchema?.enabled !== false
      ? getProductJsonLd(product, canonical, productSchema)
      : null;
