...
import { getBuildCms } from "@/lib/server/cms";
import {
  getBuildBySlug,
  listBuildSlugs,
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const cms = await getBuildCms(params.slug);
  const canonical = absoluteUrl(`/build/${build.slug}`);
  const desc = excerptPlain(cms?.seoDescription || build.summary, 165);
  const pageTitle = cms?.seoTitle || build.title;
    ...
    <article className="pb-24">
      ...
      <div className="mx-auto max-w-7xl space-y-12 px-4 pt-16 sm:px-6 lg:px-8">
        <section className="max-w-3xl space-y-4">
          ...
          {cms?.contentHtml ? (
            <div
              className="prose prose-stone max-w-none mt-6"
              dangerouslySetInnerHTML={{ __html: cms.contentHtml }}
            />
          ) : null}
        </section>

        {cms?.videoEmbeds?.length ? (
          <>
            <Separator />
            <section>
              <h2 className="font-heading text-xl tracking-wide uppercase">Video</h2>
              <div className="mt-6 grid gap-6 lg:grid-cols-2">
                {cms.videoEmbeds.map((url) => (
                  <div key={url} className="aspect-video overflow-hidden rounded-xl bg-muted">
                    <iframe
